...
let localQuizAttempts: any[] = [];
let localTasksStorage: any[] = [...DEFAULT_TASKS];
...
async function getTasks(env: Env): Promise<any[]> {
  if (env.NEXORA_KV) {
    ...
    if (data) {
      try { return JSON.parse(data); } catch (e) { return DEFAULT_TASKS; }
    }
  }
  return localTasksStorage;
}

async function saveTasks(env: Env, tasks: any[]): Promise<void> {
  if (env.NEXORA_KV) {
...
async function getAttempts(env: Env): Promise<any[]> {
  if (env.NEXORA_KV) {
...
async function saveAttempts(env: Env, attempts: any[]): Promise<void> {
  if (env.NEXORA_KV) {
...
async function generateGeminiContent(apiKey: string, body: any) {
...
3. Formulate response in detailed JSON.`;
    ...
    if (path === "/api/quiz/generate" && method === "POST") {
      const { category, difficulty, weakness } = await request.json() as any;
...
Return Taiwan Traditional Chinese (繁體中文). options strictly with A,B,C,D.`;
    ...
    if (path === "/api/quiz/attempt" && method === "POST") {
      const { questionId, questionText, chosenAnswer, correctAnswer, correct, skill } = await request.json() as any;
...
Always reply in fluent, comforting, and natural Taiwan Traditional Chinese (繁體中文).`;
    ...
    if (path === "/api/assistant/summary" && method === "POST") {
      const { profile, tasks, attempts } = await request.json() as any;
...
Return Taiwan Traditional Chinese (繁體中文).`;
    ...
    if (path === "/api/tasks" && method === "POST") {
      const { title, category, dueDate } = await request.json() as any;
      if (!title) {
